...
(function dock() {
  ...
  async function laden() {
    setStatus("verbinde …");
    try {
      const d = await api("GET", "/api/dock/verlauf?limit=50");
      verlauf.textContent = "";
      gesehen.clear();
  ...
  async function stream() {
    ...
    streamLaeuft = true;
    try {
      // fetch statt EventSource: nur so bleibt der Token im Header statt in der URL.
  ...
  async function abschicken() {
    ...
    try {
      await api("POST", "/api/dock/senden", { text: t });
      text.value = "";
  ...
  function auf() {
    ...
    setzeSeite(seite);
    toggle.classList.add("versteckt");
  ...
  function zu() {
    offen = false;
    el.classList.add("zu");
    toggle.classList.remove("versteckt");
    localStorage.setItem("dock.offen", "0");
  }

  toggle.addEventListener("click", auf);
  document.getElementById("dock-zu").addEventListener("click", zu);
  document.getElementById("dock-seite").addEventListener("click", () => {
    setzeSeite(SEITEN[(SEITEN.indexOf(seite) + 1) % SEITEN.length]);
  });
  document.addEventListener("keydown", (ev) => { if (ev.key === "Escape" && offen) zu(); });
