...
phase8_dashboard() {
  ...
  if [ "$DASH_OK" = "1" ]; then
    ...
    for F in m365.py gdrive.py mcp_m365.py vault.py mcp_n8n.py pseudonym.py pseudonym_daemon.py migrate_sessions.py llm_runner.py mcp_client.py schleuse.py werkzeuge.py anbieter.py kern.py protokoll.py mail_watch.py; do
      if [ -f "$SCRIPT_DIR/$F" ]; then cp "$SCRIPT_DIR/$F" "$BOT_DIR/$F"
